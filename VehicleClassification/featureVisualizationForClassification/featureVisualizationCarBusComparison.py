...
car_df = df_max_speed.where(df['type'] == 'Car').sample(frac=0.25)
bus_df = df_max_speed.where(df['type'] == 'Bus')
...
#use ConsecutiveZerosCount feature to compare car and bus
car_df = df.where(df['type'] == 'Car').sample(frac=0.0204)
bus_df = df.where(df['type'] == 'Bus')
car_consecutive_zeros_count = car_df['consecutiveZerosCount']
bus_consecutive_zeros_count = bus_df['consecutiveZerosCount']
plt.hist([car_consecutive_zeros_count, bus_consecutive_zeros_count])
plt.xlabel("ConsecutiveZerosCount")
plt.ylabel("Frequency")
plt.title('ConsecutiveZerosCount Comparison Histogram')
plt.savefig("consecutive_zeros_count_comparison_histogram.png")