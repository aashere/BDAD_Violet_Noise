...
object ExploratoryModelRegressionNoNoise {

  def main(args: Array[String]) = {

    // OS sensitive things
    val spark = SparkSession.builder().appName("Edge Weight Training").master("local").getOrCreate
...
//    val model_path = base_path + args(2) //"models/edgeWeightPrediction/GeneralizedLinearPoisson"
    ...
    val read_path = base_path + "features\\edgeregress"  //edgeFeatureNoise2
    // write
    val write_path = base_path + "predictions\\noNoiseEdgeWeightPrediction\\"
    val model_path = base_path + "models\\noNoiseEdgeWeightPrediction\\GeneralizedLinearPoisson"
  ...
  def ModelEvaluation(model: Transformer, filteredDF: DataFrame, pw: PrintWriter, modelName: String): Unit = {
    ...
    //df_agg.show(10)
    val agg_path = "C:\\Users\\yingl\\OneDrive\\Desktop\\Data_OLAP\\predictions\\noNoiseEdgeWeightErrorAgg\\"
