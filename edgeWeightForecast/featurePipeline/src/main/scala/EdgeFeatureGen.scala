...
object EdgeFeatureGen {
    def main(args: Array[String]) {
        ...
        val timestamps_df = (spark.sparkContext.parallelize(times_arr)
                                              .toDF("time")
                                              .withColumn("day", (col("time").cast(IntegerType) / 86400).cast(IntegerType))
                                              .withColumn("time_of_day", col("time")%86400)
                                              .drop("time")
                                              .orderBy("day","time_of_day"))

        //Take cartesian product
        val day_time_edge_df = (timestamps_df.crossJoin(edge_areas_df)
